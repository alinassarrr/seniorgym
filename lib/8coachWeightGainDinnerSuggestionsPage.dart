...
class _DinnerSuggestionsPageState extends State<DinnerSuggestionsPage> {
  ...
  void getWeightGainDinner(Function() refresh) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseURL/php/getWeightGainDinner.php'),
      ...
      if (response.statusCode == 200) {
        ...
        for (var row in jsonResponse) {
          ...
          dinnerOptions.add(row['name']);  // Store the name
        }
  ...
  void getAssignedFoods(String userID, Function() refresh) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseURL/php/getAssignedDinners.php'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8'
        },
  ...
  Future<void> addFood(String userID, String foodID) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseURL/php/addDinner.php'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8'
        },
  ...
  Future<void> deleteAssignedFoods(String userID, List<String> foodIDs) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseURL/php/deleteDinner.php'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8'
        },
